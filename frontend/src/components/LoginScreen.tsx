...
export default function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE_URL}/api/v2/admin/login`, {
        method: 'POST',
    ...
    <div className="min-h-screen bg-[#F5F5F5] flex flex-col items-center justify-center p-6 select-none font-roboto">
      <div className="bg-white border-2 border-[#111111] p-8 max-w-md w-full shadow-[6px_6px_0px_0px_rgba(17,17,17,1)]">
        <div className="flex items-center gap-3 mb-6 pb-4 border-b-2 border-[#111111]">
          <div className="p-2 bg-[#111111] border-2 border-[#111111] text-white">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-gray-500 uppercase tracking-wider font-roboto">
              Sign in to manage SMS gateway nodes
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#E50012] hover:bg-[#C4000F] text-white font-black uppercase text-xs tracking-wider py-3 border-2 border-[#111111] hover:shadow-[4px_4px_0px_0px_rgba(229,0,18,0.15)] transition-all duration-100 flex items-center justify-center gap-2 cursor-pointer font-bold"
          >
