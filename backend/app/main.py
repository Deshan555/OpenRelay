...
async def websocket_endpoint(
    ...
    try:
        while True:
            ...
            try:
                ...
                # Handle status/health report from device
                    ...
                    db.commit()
                    logger.debug(f"Status update from {device_uuid} - Battery: {battery}%, Signal: {signal}, Carrier: {carrier}, Lat: {latitude}, Lon: {longitude}")
        ...
        # Re-fetch session to avoid detached object issues across WebSocket closure
        db.refresh(device)
